...
func (s *StateDB) GetRawState(addr common.Address, hash common.Hash) []byte {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
		return stateObject.GetRawState(s.db, hash)
	}
	return nil
}
...
func (s *StateDB) GetCommittedRawState(addr common.Address, hash common.Hash) []byte {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
		return stateObject.GetCommittedRawState(s.db, hash)
	}
	return []byte{}
}
...
func (s *StateDB) SetRawState(addr common.Address, key common.Hash, value []byte) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetRawState(s.db, key, value)
	}
}
...
func (s *StateDB) populateSnapStorage(obj *StateObject) bool {
	...
	for key, value := range obj.dirtyRawStorage {
		obj.pendingRawStorage[key] = common.CopyBytes(value)
	}

	if len(obj.pendingStorage) == 0 && len(obj.pendingRawStorage) == 0 {
		return false
	}
	...
	for key, value := range obj.pendingRawStorage {
		var v []byte
