...
func (s RawStorage) String() (str string) {
	for key, value := range s {
		str += fmt.Sprintf("%X : %X\n", key, value)
	}
...
func (s RawStorage) Copy() RawStorage {
	cpy := make(RawStorage, len(s))
	for key, value := range s {
...
type StateObject struct {
	...
	originRawStorage    RawStorage

	pendingStorage Storage // Storage entries that need to be flushed to disk, at the end of an entire block
	dirtyStorage   Storage // Storage entries that have been modified in the current transaction execution
	fakeStorage    Storage // Fake storage which constructed by caller for debugging purpose.

	pendingRawStorage RawStorage // RawStorage
	dirtyRawStorage   RawStorage
	fakeRawStorage    RawStorage
...
func newObject(db *StateDB, address common.Address, data types.StateAccount) *StateObject {
	...
	return &StateObject{
		...
		originRawStorage:    make(RawStorage),
		pendingStorage:      make(Storage),
		dirtyStorage:        make(Storage),
		pendingRawStorage:   make(RawStorage),
		dirtyRawStorage:     make(RawStorage),
	}
}
...
func (s *StateObject) GetRawState(db Database, key common.Hash) []byte {

	if s.fakeRawStorage != nil {
		return common.CopyBytes(s.fakeRawStorage[key])
	}

	value, dirty := s.dirtyRawStorage[key]
	if dirty {
		return common.CopyBytes(value)
	}
	// Otherwise return the entry's original value
	return s.GetCommittedRawState(db, key)
}

func (s *StateObject) getOriginRawStorage(key common.Hash) ([]byte, bool) {
	if value, cached := s.originRawStorage[key]; cached {
...
func (s *StateObject) GetCommittedRawState(db Database, key common.Hash) []byte {
	if s.fakeRawStorage != nil {
		return common.CopyBytes(s.fakeRawStorage[key])
	}
	// If we have a pending write or clean cached, return that
	if value, pending := s.pendingRawStorage[key]; pending {
		return common.CopyBytes(value)
	}

	if value, cached := s.getOriginRawStorage(key); cached {
		return common.CopyBytes(value)
	}
...
func (s *StateObject) SetRawState(db Database, key common.Hash, value []byte) {
	// If the new value is the same as old, don't set
	prev := s.GetRawState(db, key)
	if bytes.Equal(prev, value) {
		return
	}
	// New value is different, update and journal the change
	s.db.journal.append(rawStorageChange{
		account:  &s.address,
		key:      key,
		prevalue: prev,
	})
	s.setRawState(key, value)
}
...
func (s *StateObject) setRawState(key common.Hash, value []byte) {
	s.dirtyRawStorage[key] = common.CopyBytes(value)
}
...
func (s *StateObject) finalise(prefetch bool) {
	...
	slotsToPrefetch := make([][]byte, 0, len(s.dirtyStorage)+len(s.dirtyRawStorage))
	...
	for key, value := range s.dirtyRawStorage {
		s.pendingRawStorage[key] = common.CopyBytes(value)
		if !bytes.Equal(value, s.originRawStorage[key]) {
			slotsToPrefetch = append(slotsToPrefetch, common.CopyBytes(key[:])) // Copy needed for closure
		}
	}
	...
	if len(s.dirtyRawStorage) > 0 {
		s.dirtyRawStorage = make(RawStorage)
	}
}
...
func (s *StateObject) updateTrie(db Database) Trie {
	...
	if len(s.pendingStorage) == 0 && len(s.pendingRawStorage) == 0 {
		return s.trie
	}
	...
	usedStorage := make([][]byte, 0, len(s.pendingStorage)+len(s.pendingRawStorage))
	...
	for key, value := range s.pendingRawStorage {
		if bytes.Equal(value, s.originRawStorage[key]) {
			continue
		}
		s.originRawStorage[key] = common.CopyBytes(value)
		var v []byte
	...
	if len(s.pendingRawStorage) > 0 {
		s.pendingRawStorage = make(RawStorage)
	}
...
func (s *StateObject) deepCopy(db *StateDB) *StateObject {
	...
	stateObject.dirtyRawStorage = s.dirtyRawStorage.Copy()
	stateObject.originRawStorage = s.originRawStorage.Copy()
	stateObject.pendingRawStorage = s.pendingRawStorage.Copy()
